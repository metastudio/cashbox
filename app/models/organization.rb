...
class Organization < ActiveRecord::Base
  ...
  def totals_by_customers(period)
    invoice_selection = invoices.period(period).
      select('sum(invoice_items.amount_cents) as total, invoice_items.id, invoice_items.customer_id as item_customer_id, invoice_items.currency as curr').
      joins(:invoice_items).
    ...
    selection = transactions.unscope(:order).period(period).
      ...
      where('transactions.category_id in (?) AND customers.id in (?)',
        categories.send(:incomes).pluck(:id), invoice_selection.map{ |h| h[:selection_id] }.uniq).
    ...
    selection = transactions.unscope(:order).period(period).
      ...
      joins(:customer).
      where('transactions.category_id in (?) AND customers.id in (?)',
        categories.send(:expenses).pluck(:id), invoice_selection.map{ |h| h[:selection_id] }.uniq).
      group('customers.id, bank_accounts.id').map do |transaction|
        {
          total:         transaction.total.to_f,
          selection_id:   transaction.customer_id,
    ...
    data = total_incomes.merge(expenses){ |k, v1, v2| [find_customer_name_by_id(k) + ' ' +
      Money.new((v1[1] + v2[1])*100, default_currency).format(symbol_after_without_space: true), v1[1] + v2[1]] }
