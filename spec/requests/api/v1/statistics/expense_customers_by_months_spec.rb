...
describe 'GET /api/organizations/#/statistics/income_customers_by_months' do
  let(:path)    { expense_customers_by_months_api_organization_statistic_path(org) }
  ...
  let!(:c1_expense_transactions) do
    months.each_with_object({}) do |month_beginning, collection|
      collection[month_beginning] = [
        create(:transaction, bank_account: rub_ba, customer: customer1, category: expense_category, date: month_beginning + rand(25)),
        create(:transaction, bank_account: usd_ba, customer: customer1, category: expense_category, date: month_beginning + rand(25)),
        create(:transaction, bank_account: eur_ba, customer: customer1, category: expense_category, date: month_beginning + rand(25)),
      ]
    end
  end

  let!(:c2_expense_transactions) do
    months.each_with_object({}) do |month_beginning, collection|
      collection[month_beginning] = [
        create(:transaction, :expense, bank_account: rub_ba, customer: customer2, category: expense_category, date: month_beginning + rand(25)),
        create(:transaction, :expense, bank_account: usd_ba, customer: customer2, category: expense_category, date: month_beginning + rand(25)),
        create(:transaction, :expense, bank_account: eur_ba, customer: customer2, category: expense_category, date: month_beginning + rand(25)),
      ]
    end
  end
  ...
  it 'returns expense customers statistic by months' do
    ...
    months.each_with_index do |month_beginning, i|
      ...
      c1_month_expense = c1_expense_transactions[month_beginning].sum{ |t| t.amount.exchange_to(org.default_currency) }.to_f.abs.round(2)
      c2_month_expense = c2_expense_transactions[month_beginning].sum{ |t| t.amount.exchange_to(org.default_currency) }.to_f.abs.round(2)

      expect(month_data_json.month).to eq month_beginning.strftime('%b, %Y')
      expect(month_data_json).to       include customer1.name => c1_month_expense
      expect(month_data_json).to       include customer2.name => c2_month_expense
    end
