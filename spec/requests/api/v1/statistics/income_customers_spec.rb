...
describe 'GET /api/organizations/#/statistics/income_customers' do
  let(:path)    { income_customers_api_organization_statistic_path(org) }
  ...
  it 'returns income customer statistic for current month' do
  ...
  context 'if period was provided' do
    ...
    it 'returns income categories statistic for provided period' do
      ...
      expect(statistic_json.data.map(&:to_h)).to eq([
        {
          'name'  => customer1.name,
          'value' => customer1_inc_transaction3.amount.exchange_to(org.default_currency).to_f.round(2),
        },
      ])
  ...
  context 'if no data for given preiod' do
    let!(:customer1_inc_transaction1)   {}
    let!(:customer1_inc_transaction2)   {}
    let!(:customer1_inc_transaction3)   {}
    let!(:customer2_inc_transaction1)   {}
    let!(:customer2_inc_transaction2)   {}
    let!(:customer2_exp_transaction)    {}
    let!(:exp_customer_exp_transaction) {}
